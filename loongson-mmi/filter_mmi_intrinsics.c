 ...
 *
 * Last changed in libpng 1.6.34 [April 18, 2018]
 *
...
void png_read_filter_row_sub6_mmi(png_row_infop row_info, png_bytep row,
   png_const_bytep prev_row)
{
    unsigned int bpp = (row_info->pixel_depth + 7) >> 3;
    int istop = row_info->rowbytes - bpp;
    png_bytep row_offset_ptr = row + bpp;
    png_bytep row_ptr = row;
    uint64_t num_mask;
    double row_offset_val, row_val, num_maskf;
    PNG_UNUSED(prev_row);
    __asm__ volatile (
        "dli        %[num_mask],          0x0000FFFFFFFFFFFF                          \n\t"
        "dmtc1      %[num_mask],          %[num_maskf]                                \n\t"
        "1:                                                                           \n\t"
        "daddiu     %[istop],             %[istop],                  -0x06            \n\t"
        "gsldrc1    %[row_val],           0x00(%[row_ptr])                            \n\t"
        "gsldlc1    %[row_val],           0x07(%[row_ptr])                            \n\t"
        "gsldrc1    %[row_offset_val],    0x00(%[row_offset_ptr])                     \n\t"
        "gsldlc1    %[row_offset_val],    0x07(%[row_offset_ptr])                     \n\t"
        "and        %[row_val],           %[row_val],                 %[num_maskf]    \n\t"
        "paddb      %[row_offset_val],    %[row_offset_val],          %[row_val]      \n\t"
        "gssdrc1    %[row_offset_val],    0x00(%[row_offset_ptr])                     \n\t"
        "gssdlc1    %[row_offset_val],    0x07(%[row_offset_ptr])                     \n\t"
        "daddiu     %[row_offset_ptr],    %[row_offset_ptr],          0x06            \n\t"
        "daddiu     %[row_ptr],           %[row_ptr],                 0x06            \n\t"
        "bgtz       %[istop],             1b                                          \n\t"
        : [row_offset_val]"=&f"(row_offset_val), [row_val]"=&f"(row_val),
          [num_mask]"=&r"(num_mask), [num_maskf]"=&f"(num_maskf)
        : [row_offset_ptr]"r"(row_offset_ptr), [row_ptr]"r"(row_ptr),
          [istop]"r"(istop)
        : "memory"
    );
}

void png_read_filter_row_sub8_mmi(png_row_infop row_info, png_bytep row,
   png_const_bytep prev_row)
{
     unsigned int bpp = (row_info->pixel_depth + 7) >> 3;
     int istop = row_info->rowbytes - bpp;
     png_bytep row_offset_ptr  = row + bpp;
     png_bytep row_ptr = row;
     double row_offset_val, row_val;
     PNG_UNUSED(prev_row);
     __asm__ volatile (
         "1:                                                                        \n\t"
         "daddiu     %[istop],             %[istop],                 -0x08          \n\t"
         "ldc1       %[row_val],           0x00(%[row_ptr])                         \n\t"
         "ldc1       %[row_offset_val],    0x00(%[row_offset_ptr])                  \n\t"
         "paddb      %[row_offset_val],    %[row_offset_val],         %[row_val]    \n\t"
         "sdc1       %[row_offset_val],    0x00(%[row_offset_ptr])                  \n\t"
         "daddiu     %[row_offset_ptr],    %[row_offset_ptr],         0x08          \n\t"
         "daddiu     %[row_ptr],           %[row_ptr],                0x08          \n\t"
         "bgtz       %[istop],             1b                                       \n\t"
         : [row_offset_val]"=&f"(row_offset_val), [row_val]"=&f"(row_val)
         : [row_offset_ptr]"r"(row_offset_ptr), [row_ptr]"r"(row_ptr),
           [istop]"r"(istop)
